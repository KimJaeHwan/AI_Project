...
class CNN_tensor():
    def load_images(self):
        ...
        print BEN_TEST, BEN_TRAIN
        X_train_benign = np.empty((BEN_TRAIN, 28,28,1), dtype="float32")
        y_train_benign = np.empty((BEN_TEST, ), dtype="uint8")
        X_test_benign = np.empty((BEN_TEST, 28,28,1), dtype="float32")
        y_test_benign = np.empty((BEN_TRAIN, ), dtype="uint8")
        cnt = 0
        ...
        for i in b:
            ...
            if cnt < BEN_TRAIN:
                ...
                y_test_benign[cnt-BEN_TEST,] = 0

            cnt += 1
            if cnt == (BEN_TRAIN+BEN_TEST):
                break
        m = glob.glob('../image/malware/*thumb.png')
        total_len = len(m)
        ...
        for i in m:
            ...
            if cnt < MAL_TRAIN:
                ...
                y_train_malware[cnt,] = 1
            else:
                X_test_malware[cnt-MAL_TRAIN, :, :, 0] = out
                y_test_malware[cnt-MAL_TRAIN,] = 1
