...
class Classifiers():
    ...
    def do_dnn(self):
        if "Series" in str(type(self.y_train)):
            ...
            input_len = len(self.x_train.colums)
        else:
        ...
        X = tf.compat.v1.placeholder(tf.float32,[None, input_len])
        Y = tf.compat.v1.placeholder(tf.float32,[None, 1])

        W1 = tf.Variable(tf.random.normal([input_len, 128]), name='weight1')
        b1 = tf.Variable(tf.random.truncated_normal([128]), name='bias1')
        L1 = tf.sigmoid(tf.matmul(X, W1) + b1)

        W2 = tf.Variable(tf.random.normal([128, 1024]), name='weight2')
        b2 = tf.Variable(tf.random.truncated_normal([1024]), name='bias2')
        L2 = tf.sigmoid(tf.matmul(L1, W2) + b2)
        L2 = tf.nn.dropout(L2, keep_prob)

        W3 = tf.Variable(tf.random.normal([1024,1024]), name='weight3')
        b3 = tf.Variable(tf.random.truncated_normal([1024]), name='bias3')
        L3 = tf.sigmoid(tf.matmul(L2, W3) + b3)
        L3 = tf.nn.dropout(L3, keep_prob)

        W4 = tf.Variable(tf.random.normal([1024, 128]), name='weight4')
        b4 = tf.Variable(tf.random.truncated_normal([128]), name='bias4')
        L4 = tf.sigmoid(tf.matmul(L3,W4) + b4)

        W5 = tf.Variable(tf.random.normal([128, 1]), name='weight5')
        b5 = tf.Variable(tf.random.truncated_normal([1]),name='bias5')
